...
struct Format
{
    ...
    consteval Format(const char (&t_s)[N]) : str(t_s, N - 1)
    {
        parse_and_validate();
    }
...
void info (const Format t_formatWrapper, Args&&... t_arguments)
{
    helpers::helper("INFO");
...
template <typename... Args>
void warn (const Format t_formatWrapper, Args&&... t_arguments)
{
    helpers::helper("WARN");
    helpers::print_recursive(t_formatWrapper.str, std::forward<Args>(t_arguments)...);
}
...
template <typename... Args>
void err (const Format t_formatWrapper, Args&&... t_arguments)
{
    helpers::helper("ERR");
    helpers::print_recursive(t_formatWrapper.str, std::forward<Args>(t_arguments)...);
}
