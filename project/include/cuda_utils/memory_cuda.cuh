...
{
    template <class T>
    concept cuda_pointerable_type = std::is_array_v<T> == !std::is_array_v<T> && !std::is_pointer_v<T>;

    template <cuda_pointerable_type U>
    struct cuda_deleter {
        void operator()(U* d_ptr) const noexcept {
    ...
    public:
        ...
        constexpr unique_ptr(std::nullptr_t) noexcept : mDevPtr(nullptr), mDeleter{} {}

        constexpr unique_ptr() noexcept : mDevPtr(nullptr), mDeleter{} {}
         ...
         */
        explicit unique_ptr(const pointer devPtr) noexcept : mDevPtr(devPtr), mDeleter{} {}

        unique_ptr(unique_ptr&& other) noexcept
        : mDevPtr(other.release()), mDeleter(std::move(other.get_deleter())) {}
    ...
    struct control_block {
        explicit control_block(T* devPtr) : mDeleter{}, mDevPtr(devPtr), mRefCount{1}, mWeakRefCount{1} {}
        D mDeleter;
        T* mDevPtr;
    ...
    public:
        ...
        // NOLINTNEXTLINE(google-explicit-constructor)
        constexpr shared_ptr(std::nullptr_t) noexcept : mControlBlock(nullptr), mDevPtr(nullptr) {}

        constexpr shared_ptr() : mControlBlock(nullptr), mDevPtr(nullptr) {}
         ...
         * @params devPtr already allocated (with CudaMalloc or cudaMallocPitch) pointer do device memory.
         */
        explicit constexpr shared_ptr(element_type* devPtr) noexcept : mDevPtr(devPtr) {
