...
std::vector<Move> generateAllPossibleMoves(const Board &t_board, const Colour t_color)
{
    ...
    if (pawns_attackers || kings_attackers) {
        ...
        const size_t pawns_movers = getPawnsQuietMask(pawns, empty, t_color);
...
size_t getPawnsQuietMask(const size_t t_moversPawns, const size_t t_emptyFiles, const Colour t_moversColour)
{
...
size_t getKingsAttackMask(size_t t_kingsMask, const size_t t_boardState, const size_t t_opponentsPieces)
{
    ...
    while (t_kingsMask) {
        ...
        while (possible_victims) {
            ...
            if (const Direction dir = globalTables.diffToDir[diff + 64];
                globalTables.NeighbourTable[v_idx][dir] & empty) {
                                 ...
                                 const Colour       t_attackerColour)
{
    ...
    for (const int dir : {UP_LEFT, UP_RIGHT, DOWN_LEFT, DOWN_RIGHT}) {
        ...
        if (victim_mask & t_opponentPieces && !(victim_mask & t_currentVictimsMask) && jump_mask & t_emptyFiles) {
            // update masks
            foundJump = true;
