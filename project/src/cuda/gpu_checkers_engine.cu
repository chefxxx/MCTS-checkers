...
__device__ void apply_move_gpu(GPU_Board &t_board, const GPU_Move &t_move, const Colour t_colour)
{
    // Pawn moved
    if (t_move.from_mask & t_board.pawns[t_colour]) {
        t_board.pawns[t_colour] &= ~t_move.from_mask;
        t_board.pawns[t_colour] |= t_move.to_mask & d_promotion[t_colour] ? 0ULL : t_move.to_mask;
        t_board.kings[t_colour] |= t_move.to_mask & d_promotion[t_colour] ? t_move.to_mask : 0ULL;
    }
    else {
        // King moved
